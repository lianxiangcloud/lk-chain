...
func TestStrictlyIncrementingNonces(t *testing.T) {
	...
	assert.Equal(t, 1, app.CheckTx(tx3).Code)
	assert.Equal(t, abciTypes.CodeTypeOK, app.CheckTx(tx2))

	app.BeginBlock(abciTypes.RequestBeginBlock{Hash: []byte{}, Header: &abciTypes.Header{Height: 1, Time: 1}})

	assert.Equal(t, abciTypes.CodeTypeOK, app.DeliverTx(tx1).Code)
	// expect a failure here since the nonce is not strictly increasing
	assert.Equal(t, 1, app.DeliverTx(tx3).Code)
	assert.Equal(t, abciTypes.CodeTypeOK, app.DeliverTx(tx2))
...
func TestBumpingNoncesViaRPC(t *testing.T) {
	...
	assert.Equal(t, 1, app.CheckTx(tx1).Code)
	// ...on both interfaces of the app
	assert.Equal(t, core.ErrNonceTooLow, backend.Ethereum().TxPool().AddLocal(rawTx1))

	assert.Equal(t, nil, backend.Ethereum().TxPool().AddLocal(rawTx2))
...
func TestMultipleTxOneAcc(t *testing.T) {
	...
	assert.Equal(t, abciTypes.CodeTypeOK, app.DeliverTx(tx1))
	assert.Equal(t, abciTypes.CodeTypeOK, app.DeliverTx(tx2))
