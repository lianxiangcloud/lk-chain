...
func (blockExec *BlockExecutor) SetSwitch(sw *p2p.Switch) {
	blockExec.sw = sw
}
...
func (blockExec *BlockExecutor) ApplyBlock(s State, blockID types.BlockID, block *types.Block) (State, error) {
	...
	if blockExec.sw != nil {
		validators, peers, _ := blockExec.sw.GetNumPeersByRole()
		lklog.InfoData("70001\x01height,validators,peers\x01%d\x01%d\x01%d", block.Height, validators, peers)
	}
