...
func GetLocalAllAddress() []string {
	...
	if err != nil {
		panic(cmn.Fmt("Could not get local addresses: %v", err))
	}

	var privateAddr10 []string
	var privateAddr127 []string
	for _, a := range addrs {
		...
		if v4[0] == 127{
			privateAddr127 = append(privateAddr127, ipnet.IP.String())
			continue
		}
		if v4[0] == 10 {
			privateAddr10 = append(privateAddr10, ipnet.IP.String())
			continue
		}

		localaddr = append(localaddr, ipnet.IP.String())
	}
	if len(localaddr) > 0 {
		return localaddr
	}
	if len(privateAddr10) > 0 {
		return privateAddr10
	}
	return privateAddr127
}
...
func DelLocalAddress(addrs []string) []string {
	...
	localaddrs := GetLocalAllAddress()
	for _, addr := range addrs {
		host, _, err := net.SplitHostPort(removeProtocolIfDefined(addr))
		if err != nil {
			continue
		}
