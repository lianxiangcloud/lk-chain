...
type Switch struct {
	...
	connectAddrs    map[string]bool
	connectAddrsMtx sync.Mutex
}
...
func NewSwitch(config *cfg.P2PConfig) *Switch {
	sw := &Switch{
		...
		dialing:      cmn.NewCMap(),
		nodeInfo:     nil,
		connectAddrs: make(map[string]bool),
	}
...
func (sw *Switch) SetNodeInfo(nodeInfo *NodeInfo) {
	...
	if sw.nodeInfo != nil {
		sw.connectAddrsMtx.Lock()
		for _, addr := range sw.nodeInfo.LocalAddrs {
			sw.connectAddrs[addr] = true
		}
		sw.connectAddrsMtx.Unlock()
	}
}
...
func (sw *Switch) GetNumPeersByRole() (int, int, int) {
	...
	for _, peer := range swPeers {
		if peer.Type() == "validator" {
			validators++
		} else if peer.Type() == "peer" {
			peers++
		} else {
			// WARN: Wrong nodeType
		}
...
func (sw *Switch) OnStop() {
	...
	for _, peer := range sw.peers.List() {
		...
		peerAddrs := peer.GetAddrs()
		sw.connectAddrsMtx.Lock()
		for _, addr := range peerAddrs {
			delete(sw.connectAddrs, addr)
		}
		sw.connectAddrsMtx.Unlock()
	}
	// Stop reactors
...
func (sw *Switch) addPeer(peer *peer) error {
	...
	sw.Logger.Info("Added peer success", "peer", peer)
	return nil
}
...
func (sw *Switch) startInitPeer(peer *peer) {
	...
	sw.Logger.Info("reactor Add Peer")
	for _, reactor := range sw.reactors {
		reactor.AddPeer(peer)
	}
	sw.Logger.Info("reactor Add Peer success")
}
...
func (sw *Switch) DialSeeds(addrBook *AddrBook, seeds []string) error {
	...
	sw.Logger.Info("DialSeeds: ", "seeds", seeds)

	if addrBook != nil {
		// add seeds to `addrBook`
		ourAddrS := sw.nodeInfo.ListenAddr
		ourAddr, err := NewNetAddressString(ourAddrS)
		if err != nil {
			sw.Logger.Error("DialSeeds: NewNetAddressString", "err", err, "ourAddrs", ourAddrS)
			return fmt.Errorf("DialSeeds: NewNetAddressString: addr=%s, err=%v", ourAddrS, err)
		}
...
func (sw *Switch) StopPeerForError(peer Peer, reason interface{}) {
	...
	if peer.IsPersistent() && (!sw.HasAddr(peer.NodeInfo().RemoteAddr)) {
