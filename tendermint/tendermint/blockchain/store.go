...
func (bs *BlockStore) SaveBlock(block *types.Block, blockParts *types.PartSet, seenCommit *types.Commit) {
	...
	if !blockParts.IsComplete() {
		var msg = struct {
			...
			Module: "tendermint",
			Reason: fmt.Sprintf("BlockStore: consensus failure, incomplete block part sets, %s", blockParts.String()),
			BlockHeader: block.Header,

		}
		reporttrans.WarnJson(reporttrans.IdTendermintConsensusFailure, msg)
...
func (bs *BlockStore) DeleteBlock(height int64) {
	var n int
	var err error
	r := bs.GetReader(calcBlockMetaKey(height))
	if r == nil {
	...
	//fmt.Printf("Delete block %d done\n", height)
	// Flush
	bs.db.SetSync(nil, nil)
}
