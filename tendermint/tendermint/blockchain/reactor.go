...
type mempoolReactor interface {
	// when blockchain reactor work over, notify mempool send tx info to
	// consensus when new tx received
	NotifyTx()
}
	...
	for {
		...
		SYNC_LOOP:
			for i := 0; i < 10; i++ {
				...
				if err != nil {
					bcR.Logger.Error("VeriyfyCommit: Error in validation", "err", err, "chainID", chainID,
						"firstID", firstID, "first.Height", first.Height, "second.Height",
						second.Height, "second.LastCommit", second.LastCommit)
