...
func NewPartSetFromData(data []byte, partSize int) *PartSet {
	...
	//	fmt.Printf("\nNewPartSetFromData: dataSize=%d, partSize=%d, total=%d, hashRoot=%v\n", len(data), partSize, total, root)
	//	for i := 0; i < total; i++ {
	//		fmt.Printf("NewPartSetFromData: index=%d, part=%v\n", i, parts[i])
	//	}
	//	fmt.Println()
...
func (ps *PartSet) String() string {
	...
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	return fmt.Sprintf("{count=%d, total=%d, parts=%d}", ps.Count(), ps.Total(), len(ps.parts))
}
