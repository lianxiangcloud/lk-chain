...
func (valSet *ValidatorSet) VerifyCommit(chainID string, blockID BlockID, height int64, commit *Commit) error {
	...
	//fmt.Printf("\nVerifyCommit: precommits=%v, valset=%v, totalPower=%d\n\n", commit.Precommits, valSet.Validators, valSet.TotalVotingPower())
	for idx, precommit := range commit.Precommits {
		...
		if !blockID.Equals(precommit.BlockID) {
			fmt.Printf("\nblockID=%v, precommit.BlockID=%v, precommit.Height=%d\n\n", blockID, precommit.BlockID, precommit.Height)
			continue // Not an error, but doesn't count
		}
		// fmt.Printf("tailliedVotingPower: addPower, val.pubkey=%s, val=%v, power=%d\n", val.PubKey, val, val.VotingPower)
		// Good precommit!
