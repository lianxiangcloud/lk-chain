...
func New(ctx *node.ServiceContext, config *Config, pending miner.Pending) (*Ethereum, error) {
	...
	log.Info("Initialising Ethereum protocol", "versions", ProtocolVersions, "network", config.NetworkId, "chainconfig", chainConfig.String())
	...
	eth.txPool = core.NewTxPool(config.TxPool, eth.chainConfig, eth.blockchain, eth.blockchain.IsAccountLockup)
...
func (s *Ethereum) APIs() []rpc.API {
	apis := ethapi.GetAPIs(s.ApiBackend, s.config.PocketAPI)
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	log.Info("GetLogs: ethereum.Start: start service")
