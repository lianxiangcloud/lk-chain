...
// IsAccoutLockup returns whether the account is lockup
func (bc *BlockChain) IsAccountLockup(from common.Address) bool {
	bc.mu.RLock()
	defer bc.mu.RUnlock()
	return ethash.IsAccountLockup(from, bc.currentBlock.Number())
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if _, err := state.New(currentBlock.Root(), bc.stateCache); err != nil {
		...
		log.Warn("Head state missing, resetting chain", "number", currentBlock.Number(), "hash", currentBlock.Hash(),
			"root", currentBlock.Root().String(), "header", currentBlock.Header().String())
		// @Note: for debug, do not reset.
		//panic("Head state missing, please check db.")
		return bc.Reset()
	}
...
func (bc *BlockChain) DeleteBlock(hash common.Hash, number uint64) {
	DeleteBlock(bc.chainDb, hash, number)
	bc.blockCache.Remove(hash)
}

func (bc *BlockChain) RollbackBlock(number uint64) {
	hash := GetCanonicalHash(bc.chainDb, number)
	if hash == (common.Hash{}) {
		return
	}

	log.Warn("RollbackBlock", "number", number, "hash", hash)
	...
	blk := bc.GetBlock(bc.currentFastBlock.ParentHash(), bc.currentFastBlock.NumberU64()-1)
	if blk != nil {
		bc.currentFastBlock = blk
		WriteHeadFastBlockHash(bc.chainDb, bc.currentFastBlock.Hash())
	}

	blk = bc.GetBlock(bc.currentBlock.ParentHash(), bc.currentBlock.NumberU64()-1)
	if blk != nil {
		bc.currentBlock = blk
		WriteHeadBlockHash(bc.chainDb, bc.currentBlock.Hash())
	}

	bc.DeleteBlock(hash, number)
}

func (bc *BlockChain) RollbackToBlockNumber(n uint64) {
	height := bc.hc.CurrentHeader().Number.Uint64()
	if n >= height {
		log.Error("RollbackToBlockNumber failed", "blocknumber", n, "height", height)
		os.Exit(1)
	}

	log.Warn("RollbackToBlockNumber", "blocknumber", n, "height", height)
...
func SetReceiptsData(config *params.ChainConfig, block *types.Block, receipts types.Receipts) {
	...
	for j := 0; j < len(receipts); j++ {
		...
		if transactions[j].To() == nil {
			...
			receipts[j].ContractAddress = crypto.CreateAddress(from, transactions[j].Nonce(), transactions[j].Data())
		}
...
func (bc *BlockChain) PostChainEventsByBlocks(chain types.Blocks, status WriteStatus) error {
	// Pre-checks passed, start the full block imports
	bc.wg.Add(1)
	defer bc.wg.Done()
	...
	log.Debug("PostChainEventsByBlocks", "eventslen", len(events), "logslen", len(coalescedLogs))
