...
import (
	...
	otTypes "third_part/txmgr/types"
)
...
func opCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if err != nil {
		...
		if evm.tb != nil {
			// @Note save Contract Tranfer Transaction
			ctx := otTypes.ContractTransaction{
				TxHash: evm.StateDB.TxHash(),
				From:   contract.Address(),
				To:     toAddr,
				Value:  value,
			}
			lkx := otTypes.LKTransaction{
				Type: otTypes.ContractTxType,
				CTx:  &ctx,
			}
			if err := evm.tb.Put(&lkx); err != nil {
				log.Error("opCall TxBatch.Put fail", "tx_hash", ctx.TxHash, "err", err)
			} else {
				log.Debug("opCall TxBatch.Put ok", "ctx", &ctx)
			}
		} else {
			log.Debug("TxBatch nil")
		}
	}
...
func opSuicide(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	evm.StateDB.AddBalance(to, balance)

	evm.StateDB.Suicide(contract.Address())
	// save Suicide
	if evm.tb != nil {
		stx := otTypes.SuicideTransaction{
			TxHash: evm.StateDB.TxHash(),
			From:   contract.Address(),
			To:     to,
			Target: to,
		...
		lkx := otTypes.LKTransaction{
			Type: otTypes.SuicideTxType,
			STx:  &stx,
		}
		if err := evm.tb.Put(&lkx); err != nil {
			log.Error("opSuicide TxBatch.Put fail", "tx_hash", stx.TxHash, "err", err)
		} else {
			log.Debug("opSuicide TxBatch.Put ok", "stx", &stx)
		}
	} else {
		log.Debug("TxBatch nil")
	}
