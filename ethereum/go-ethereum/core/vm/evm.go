...
func NewEVM(ctx Context, statedb StateDB, chainConfig *params.ChainConfig, vmConfig Config, tb *lkdb.TxBatch) *EVM {
	evm := &EVM{
		Context:     ctx,
		StateDB:     statedb,
...
func (evm *EVM) Reset(origin common.Address, gasPrice *big.Int) {
	evm.depth = 0
	evm.abort = 0
	evm.callGasTemp = 0
	// @Todo: ywh
	// evm.OTxs = make([]types.Tx, 0)

	evm.Context.Origin = origin
	evm.Context.GasPrice = gasPrice
