...
// rotateLogger defines the behavior of a log provider.
...
func NoSyncStreamHandler(rl rotateLogger, fmtr Format) Handler {
	h := FuncHandler(func(r *Record) error {
		err := rl.Write(fmtr.Format(r))
		return err
	})
	return LazyHandler(h)
}

func RotateHandler(path string, fmtr Format) (Handler, error) {
	lg := newFileWriter()
	...
	return NoSyncStreamHandler(lg, fmtr), nil
}
...
func (w *fileLogWriter) doRotate(logTime time.Time) error {
	...
	go w.deleteOldLog() // hucc 这里可以控制清理频率
