...
func newLogWriter(wr io.Writer, prefix string) *logWriter {
	lg := &logWriter{writer: wr, prefix: prefix}
	lg.hasLeaders = len(lg.prefix) > 0
	return lg
}
...
func (lg *logWriter) println(when time.Time, msg string) {
	...
	if lg.hasLeaders {
		h = append([]byte(lg.prefix + "\x01"), h...)
	}
	lg.writer.Write(append(h, msg...))
	lg.Unlock()
}
